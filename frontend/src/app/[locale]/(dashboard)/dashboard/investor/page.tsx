...
import { projects, sectorOptions, sectorLabels } from "@/lib/mock-data";
import { cn, formatNumber } from "@/lib/utils";

type InvestorTab = "discover" | "saved" | "interests" | "preferences";
...
const initialInterests: InterestRow[] = [
...
export default function InvestorDashboardPage() {
  ...
  const [tab, setTab] = useState<InvestorTab>("discover");
  const [saved, setSaved] = useState<string[]>(["p1", "p4", "p8"]);
  const [interests] = useState(initialInterests);
  const [sectorFilter, setSectorFilter] = useState<string>("all");
  const [prefSectors, setPrefSectors] = useState<string[]>(["ai_ml", "fintech"]);
  ...
  function togglePrefSector(sector: string) {
    setPrefSectors((prev) =>
      prev.includes(sector) ? prev.filter((s) => s !== sector) : [...prev, sector]
    );
  }

  const tabs: { key: InvestorTab; label: string }[] = [
